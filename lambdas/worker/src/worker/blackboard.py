"""DynamoDB CRUD helpers for the Worker bounded context."""
...
class Blackboard:
    """DynamoDB wrapper for the Worker bounded context."""
    ...
    def query_gsi(
        ...
        if sk_prefix:
            resp = self._table.query(
                ...
                KeyConditionExpression=DKey("GSI1PK").eq(pk)
                & DKey("GSI1SK").begins_with(sk_prefix),
            )
        else:
            resp = self._table.query(
                IndexName=index_name,
